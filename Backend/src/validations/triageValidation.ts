...
export const createTriageSchema = z.object({
  appointmentId: z.number().int().positive('Appointment ID must be a positive integer').optional().nullable(),
...
export const updateTriageSchema = z.object({
  ...
  requiredSpecialtyId: z.number().int().positive().optional().nullable()
});
